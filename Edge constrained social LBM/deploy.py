...
# ReferenceSubscriber lives outside this repo (LBM research project).
_REF_SUB_DIR = Path("/home/eduardot/Documents/Research/Masters research/Edge constrained social LBM")
if str(_REF_SUB_DIR) not in sys.path:
    sys.path.insert(0, str(_REF_SUB_DIR))
from reference_subscriber import ReferenceSubscriber  # noqa: E402

# Flat-obs layout for TrackingPolicy (see src/observation.py + src/policy.py):
# BootIndicator(1) | TrackingCommandObs[root_height(5) pos_diff(8) heading(8) force(1) tgt_joints(145) g_local(15)] | ...
# The t=0 reference-joint slot sits at [23, 52); full 5-frame ref block is [23, 168).
REF_JOINTS_START = 23
REF_JOINTS_PER_FRAME = 29

# Indices into the 29-DoF isaac_joint_names_state ordering for the arms only
# (shoulders, elbows, wrists). Injecting only these keeps the legs and waist
# at the loaded motion's default standing pose, so the robot performs the
# handshake without trying to walk.
...
class Controller:
    def __init__(self, args, ctrl_cfg):
        ...
        self.ref_sub: Optional[ReferenceSubscriber] = None
    ...
    def default_qpos_state(self):
        ...
        # ZMQ reference-joint subscriber: pre-arm with the motion's default
        # standing pose so the held frame is never zeros, and gate injection
        # on has_frame so nothing overwrites until the orchestrator fires.
        default_ref = self.current_policy.ref_joint_pos[self.current_policy.ref_idx].astype(np.float32).copy()
        self.ref_sub = ReferenceSubscriber(initial=default_ref)
        print(f"[Controller] ReferenceSubscriber armed (held frame = current default ref)")
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # Inject the live ZMQ reference into ALL 5 lookahead slots
                # (t=0,2,4,8,16) for the arm joints only. Leg and waist slots
                # keep the loaded motion's default standing values so the
                # robot performs the handshake without walking.
                frame = self.ref_sub.poll()  # (29,) float32, never None
                if self.ref_sub.has_frame:
                    obs = self.current_policy.policy_input["policy"]  # (1, num_obs)
                    arm_vals = frame[ARM_JOINT_INDICES]
                    for slot in range(5):
                        base = REF_JOINTS_START + slot * REF_JOINTS_PER_FRAME
                        obs[0, base + ARM_JOINT_INDICES] = arm_vals
